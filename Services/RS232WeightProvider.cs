...
{
    /// <summary>
    /// RS-232 Weight provider for processing weight data from scale devices
    /// Based on TaroziKlass project implementation
    /// Uses independent reading task that NEVER stops
    /// </summary>
    public class RS232WeightProvider : IDisposable
    {
        ...
        public RS232WeightProvider() { }

        public RS232WeightProvider(PaymentState? paymentState = null)
        {
            _paymentState = paymentState;
        }

        /// <summary>
        /// Starts reading weight data from the RS-232 port
        /// Reading task runs independently and NEVER stops
        /// </summary>
        /// <param name="portName">Serial port name (e.g., "COM3" on Windows or "/dev/ttyS7" on Linux)</param>
        public void Start(string portName = "/dev/ttyS7", int baudRate = 9600, int dataBits = 8, int stopBits = 1, AndroidSerialPort.Parity parity = AndroidSerialPort.Parity.None)
        {
            lock (_lockObject)
            {
                _baudRate = baudRate;
                ...
                try
                {
                    ...
                    if (_fd >= 0)
                    {
                        System.Diagnostics.Debug.WriteLine($"[RS232] Port opened: {portName}, fd: {_fd}");
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"[RS232] Failed to open port {portName}");
                    }
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"[RS232] Connect error: {ex.Message}");
                    _fd = -1;
                }
            }
        }
        ...
        private void StartReading()
        {
            lock (_lockObject)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("[RS232] Reading task already running, skipping start");
                    return;
                }
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[RS232] Reading task started on thread {Thread.CurrentThread.ManagedThreadId} - NEVER STOPS");
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            if (currentFd >= 0)
                            {
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            System.Diagnostics.Debug.WriteLine($"[RS232] No data for 3s, clearing weight.");
                                ...
                                {
                                    System.Diagnostics.Debug.WriteLine($"[RS232] Read error (port may be closed): {ex.Message}");
                            ...
                            else
                            {
                                ...
                                System.Diagnostics.Debug.WriteLine("[RS232] Port not open, waiting for connection...");
                        ...
                        {
                            System.Diagnostics.Debug.WriteLine($"[RS232] Unexpected error in reading loop: {ex.Message}");
        ...
        public void Stop()
        {
            ...
            lock (_lockObject)
            {
                if (_fd >= 0)
                {
                    ...
                    {
                        AndroidSerialPort.CloseFd(_fd);
                        System.Diagnostics.Debug.WriteLine("[RS232] Port closed, but reading task continues");
                    }
                    catch (Exception ex)
                    {
                        System.Diagnostics.Debug.WriteLine($"[RS232] Error closing port: {ex.Message}");
                    }
                    _fd = -1;
                }
            }
        ...
        public void Dispose()
        {
            if (!_disposed)
            {
                lock (_lockObject)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine("[RS232] Service disposed, but reading task continues running");
                }
            }
        }
    }
